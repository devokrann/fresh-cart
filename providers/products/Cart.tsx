...
export default function Cart({
	...
	const removeFromCart = (set: typeProductVariant[]) => {
		...
		if (itemsToRemove.length > 0) {
			...
			setCart(cart.filter(p => !compoundIdsToRemove.includes(p.id)));
	...
	const updateQuantity = {
		decrease(id: string) {
			setCart(cart.map(p => (p.id == id ? { ...p, quantity: p.quantity > 1 ? p.quantity - 1 : p.quantity } : p)));
		},
